...
function getCityList(conn,orgLat,orgLong,taste,range,start,batchsize,callback) {
	...
	var queryString='SELECT CityName,CityID,CityName as AlternateName, Latitude,Longitude,State, Description, Rating,CityImage as Image,( 6371 * acos( cos( radians('+orgLat+') ) * cos( radians( Latitude ) ) * cos( radians( Longitude ) - radians('+orgLong+') ) + sin( radians('+orgLat+') ) * sin( radians( Latitude ) ) ) ) AS distance FROM City WHERE '+subQuery+' HAVING distance < '+range+' AND distance > 20 ORDER BY Rating DESC LIMIT '+ connection.escape(start) +', '+ connection.escape(batchsize)+';';
