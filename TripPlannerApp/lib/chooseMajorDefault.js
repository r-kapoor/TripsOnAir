...
function chooseMajorDefault(rome2RioDataArray, dates, times,budget, callbackResponse)
{
	...
	if((withTaxiRome2rioData.TravelBudget<(budget*budgetFactor))&&(getDuration.combineDatesTimes(dates, times)[1].isAfter(withTaxiRome2rioData.endTimeOfTrip)))
	{
		console.log("1");
		withTaxiRome2rioData.isMajorDefault=1;
		withoutTaxiRome2rioData.isMajorDefault=0;
	}
	else if((withoutTaxiRome2rioData.TravelBudget<(budget*budgetFactor))&&(getDuration.combineDatesTimes(dates, times)[1].isAfter(withoutTaxiRome2rioData.endTimeOfTrip)))
	{
		console.log("2");
